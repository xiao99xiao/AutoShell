...
- **Periodic restarts:** A dedicated native settings sheet enables a per-task interval in seconds, minutes, hours, or days. It can be changed while running without editing the command. Task details show the interval and next restart date, or a paused/off state. Manual stops cancel the current timer; the interval persists for the next start.
